...
def get_array_dimensions(data):
    ...
    dimensions = tuple(
        ...
        for _depth, dimensions in sorted(
            grouped_by_depth.items(), reverse=True
        )
    )
...
def encode_field(types, name, field_type, value):
    if value is None:
        raise ValueError(
            f"Missing value for field {name} of type {field_type}"
        )
    ...
    if not is_encodable_type(field_type):
        raise TypeError(
            f"Received Invalid type `{field_type}` in field `{name}`"
        )
