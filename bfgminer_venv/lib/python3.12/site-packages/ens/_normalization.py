...
specs_dir_path = os.path.abspath(
...
VALID_BY_GROUPS = {
    d["name"]: set(d["primary"] + d["secondary"])
    for d in NORMALIZATION_SPEC["groups"]
}
...
def _codepoints_to_text(cps: Union[List[List[int]], List[int]]) -> str:
    return "".join(
        chr(cp) if isinstance(cp, int) else _codepoints_to_text(cp)
        for cp in cps
    )
...
def _validate_tokens_and_get_label_type(tokens: List[Token]) -> str:
    ...
    if len(tokens) == 1 and tokens[0].type == TokenType.TEXT:
        ...
        try:
            ...
            if (
                "_"
                in concat_text_tokens_as_str[
                    concat_text_tokens_as_str.count("_") :
                ]
            ):
                raise InvalidName(
    ...
    for group in NORMALIZATION_SPEC["groups"]:
        if group["name"] == chars_group_name:
            if "cm" not in group:
                nfd_cps = [
                    ord(nfd_c)
                    for c in concat_text_tokens_as_str
                    for nfd_c in NFD(c)
                ]
                ...
                for cp_i, cp in enumerate(nfd_cps):
                    ...
                    if cp in NORMALIZATION_SPEC["nsm"]:
                        ...
                        if not len(contiguous_nsm_cps) == len(
                            set(contiguous_nsm_cps)
                        ):
                            raise InvalidName(
    ...
    for char_cp in text_token_cps_set:
        ...
        if groups_excluding_ce and len(groups_excluding_ce) > 0:
            if len(retained_groups) == 0:
                ...
                retained_groups = retained_groups.intersection(
                    groups_excluding_ce
                )
    ...
    if len(confused_chars) > 0:
        for retained_group_name in retained_groups:
            if all(
                cp in VALID_BY_GROUPS[retained_group_name] for cp in buffer
            ):
...
def _buffer_codepoints_to_chars(
    buffer: Union[List[int], List[List[int]]],
) -> str:
    return "".join(
