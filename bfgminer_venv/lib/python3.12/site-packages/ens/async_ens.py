...
class AsyncENS(BaseENS):
    ...
    @classmethod
    def from_web3(
        cls, w3: "AsyncWeb3", addr: ChecksumAddress = None
    ) -> "AsyncENS":
        """
    ...
    async def setup_address(
        ...
        if coin_type is None:
            return await resolver.functions.setAddr(node, address).transact(
                transact
            )
        else:
            return await resolver.functions.setAddr(
                node, coin_type, address
            ).transact(transact)
    ...
    async def name(self, address: ChecksumAddress) -> Optional[str]:
        ...
        return (
            name
            if to_checksum_address(address) == await self.address(name)
            else None
        )
    ...
    async def setup_name(
        ...
        if not name:
            ...
            if is_none_or_zero_address(address):
                ...
                resolved and address != resolved and resolved != EMPTY_ADDR_HEX
            ):
    ...
    async def _set_resolver(
        ...
        return cast(
            "AsyncContract", self._resolver_contract(address=resolver_addr)
        )
    ...
    async def _resolve(
        ...
        if await _async_resolver_supports_interface(
            ...
            return (
                to_checksum_address(result) if is_address(result) else result
            )
    ...
    async def _setup_reverse(
        ...
        return await reverse_registrar.functions.setName(name).transact(
            transact
        )
...
async def _async_validate_resolver_and_interface_id(
    ...
    elif not await _async_resolver_supports_interface(
        resolver, ens_interface_id
    ):
