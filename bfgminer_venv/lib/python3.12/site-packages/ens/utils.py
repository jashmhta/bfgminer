...
def init_web3(
    ...
    if provider is default:
        ...
        w3 = Web3Main(
            provider, middlewares, ens=None, modules={"eth": (EthMain)}
        )
...
def ens_encode_name(name: str) -> bytes:
    ...
    dns_prepped_labels = [
        to_bytes(len(label)) + label for label in labels_as_bytes
    ]
...
def label_to_hash(label: str) -> HexBytes:
    ...
    if "." in label:
        raise ValueError(
            f"Cannot generate hash for label {label!r} with a '.'"
        )
...
def estimate_auction_start_gas(labels: Collection[str]) -> int:
    return AUCTION_START_GAS_CONSTANT + AUCTION_START_GAS_MARGINAL * len(
        labels
    )
...
async def _async_ens_stalecheck_middleware(
    ...
    middleware = await async_make_stalecheck_middleware(
