...
class FixedWindowRateLimiter(RateLimiter):
    ...
    def test(
        ...
        return (
            self.storage.get(item.key_for(*identifiers))
            < item.amount - cost + 1
        )
...
class SlidingWindowCounterRateLimiter(RateLimiter):
    ...
    def _weighted_count(
        ...
        """
        return (
            previous_count * previous_expires_in / item.get_expiry()
            + current_count
        )
    ...
    def get_window_stats(
        ...
        (
            ...
            current_count,
            current_expires_in,
        ) = cast(SlidingWindowCounterSupport, self.storage).get_sliding_window(
        ...
        return WindowStats(
            now + min(previous_reset_in, current_reset_in), remaining
        )
