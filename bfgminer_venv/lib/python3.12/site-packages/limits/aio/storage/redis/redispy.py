...
class RedispyBridge(RedisBridge):
    ...
    def use_cluster(self, **options: str | float | bool) -> None:
        ...
        for loc in self.parsed_uri.netloc[sep:].split(","):
            ...
            cluster_hosts.append(
                self.dependency.asyncio.cluster.ClusterNode(
    ...
    def register_scripts(self) -> None:
        ...
        self.lua_acquire_sliding_window = (
            self.get_connection().register_script(
    ...
    async def get_moving_window(
        ...
        key = self.prefixed_key(key)
        timestamp = time.time()
        window = await self.lua_moving_window(
    ...
    async def get_sliding_window(
        ...
        if window := await self.lua_sliding_window(
            [self.prefixed_key(previous_key), self.prefixed_key(current_key)],
            [expiry],
        ):
            return (
                int(window[0] or 0),
    ...
    async def reset(self) -> int | None:
        ...
        keys = await self.storage.keys(
            prefix,
            target_nodes=self.dependency.asyncio.cluster.RedisCluster.ALL_NODES,
        )
        count = 0
        for key in keys:
