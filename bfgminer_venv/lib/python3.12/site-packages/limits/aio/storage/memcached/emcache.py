...
class EmcacheBridge(MemcachedBridge):
    ...
    async def get_storage(self) -> emcache.Client:
        if not self._storage:
            self._storage = await self.dependency.create_client(
                [
                    self.dependency.MemcachedHostAddress(h, p)
    ...
    async def get_many(self, keys: Iterable[str]) -> dict[bytes, int]:
        ...
        return {
            k: int(item.value) if item else 0 for k, item in results.items()
        }
    ...
    async def decr(
        self, key: str, amount: int = 1, noreply: bool = False
    ) -> int:
        storage = await self.get_storage()
        limit_key = key.encode("utf-8")
        try:
            value = (
                await storage.decrement(limit_key, amount, noreply=noreply)
                or 0
            )
        except self.dependency.NotFoundCommandError:
            value = 0
    ...
    async def incr(
        ...
        try:
            ...
            try:
                await storage.add(
                    limit_key, f"{amount}".encode(), exptime=ceil(expiry)
                )
