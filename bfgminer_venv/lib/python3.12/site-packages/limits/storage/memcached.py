...
class MemcachedStorage(
    Storage, SlidingWindowCounterSupport, TimestampedSlidingWindow
):
    """
    ...
    def storage(self) -> MemcachedClientP:
        ...
        if not (
            ...
            if not dependency:
                raise ConfigurationError(
                    f"Unable to import {self.cluster_library}"
                )
    ...
    def get_expiry(self, key: str) -> float:
        ...
        return float(
            self.storage.get(self._expiration_key(key)) or time.time()
        )
    ...
    def acquire_sliding_window_entry(
        ...
        previous_count, previous_ttl, current_count, _ = (
            self._get_sliding_window_info(
        ...
        if floor(weighted_count) + amount > limit:
            ...
            current_count = self.incr(
                ...
                2 * expiry,
                amount=amount,
                set_expiration_key=False,
            )
            actualised_previous_ttl = min(
                0, previous_ttl - (time.time() - now)
            )
            weighted_count = (
                previous_count * actualised_previous_ttl / expiry
                + current_count
            )
    ...
    def get_sliding_window(
        ...
        now = time.time()
        previous_key, current_key = self.sliding_window_keys(key, expiry, now)
        return self._get_sliding_window_info(
