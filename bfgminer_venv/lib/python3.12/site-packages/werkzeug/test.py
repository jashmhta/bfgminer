    ...
    for key, value in _iter_data(data):
        ...
        if reader is not None:
            ...
            while True:
                ...
                if not chunk:
                    write_binary(
                        encoder.send_event(Data(data=chunk, more_data=False))
                    )
                    break
            ...
            write_binary(
                encoder.send_event(Data(data=value.encode(), more_data=False))
            )
...
class EnvironBuilder:
    ...
    def __init__(
        ...
        data: None | (
            t.IO[bytes] | str | bytes | t.Mapping[str, t.Any]
        ) = None,
    ...
    def _add_file_from_data(
        ...
        value: (
            t.IO[bytes]
            | tuple[t.IO[bytes], str]
            | tuple[t.IO[bytes], str, str]
        ),
    ) -> None:
    ...
    def _make_base_url(scheme: str, host: str, script_root: str) -> str:
        return (
            urlunsplit((scheme, host, script_root, "", "")).rstrip("/") + "/"
        )
    ...
    def _get_form(
        self, name: str, storage: type[_TAnyMultiDict]
    ) -> _TAnyMultiDict:
    ...
    def set_cookie(
        ...
        **kwargs: t.Any,
        ...
        cookie = Cookie._from_response_header(
            domain,
            "/",
            dump_cookie(key, value, domain=domain, path=path, **kwargs),
        )
...
class Cookie:
    ...
    def _matches_request(self, server_name: str, path: str) -> bool:
        return (
            ...
            or (
                ...
                and path[
                    len(self.path) - self.path.endswith("/") :
                ].startswith("/")
            )
        )
    ...
    def _from_response_header(
        ...
        return cls(
            ...
            max_age=(
                int(params["max-age"] or 0) if "max-age" in params else None
            ),
