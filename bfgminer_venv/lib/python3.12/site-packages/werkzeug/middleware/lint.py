...
class GuardedWrite:
    def __init__(
        self, write: t.Callable[[bytes], object], chunks: list[int]
    ) -> None:
        self._write = write
        self._chunks = chunks
...
class LintMiddleware:
    ...
    def check_environ(self, environ: WSGIEnvironment) -> None:
        ...
        if environ["wsgi.version"] != (1, 0):
            warn(
                "Environ is not a WSGI 1.0 environ.", WSGIWarning, stacklevel=3
            )
    ...
    def check_start_response(
        ...
        if len(status_code_str) != 3 or not status_code_str.isdecimal():
            warn(
                "Status code must be three digits.", WSGIWarning, stacklevel=3
            )
    ...
    def __call__(self, *args: t.Any, **kwargs: t.Any) -> t.Iterable[bytes]:
        ...
        def checking_start_response(
            *args: t.Any, **kwargs: t.Any
            ...
            return GuardedWrite(
                start_response(status, headers, exc_info), chunks
            )

        app_iter = self.app(
            environ, t.cast("StartResponse", checking_start_response)
        )
