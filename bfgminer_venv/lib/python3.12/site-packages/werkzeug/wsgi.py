...
from .sansio.utils import (
    host_is_trusted,
)  # noqa: F401 # Imported as part of API
    ...
    if not host_only:
        ...
        if not root_only:
            ...
            if not strip_querystring:
                parts["query_string"] = environ.get("QUERY_STRING", "").encode(
                    "latin1"
                )
...
def get_input_stream(
    ...
    if "wsgi.input_terminated" in environ:
        if max_content_length is not None:
            ...
            return t.cast(
                t.IO[bytes],
                LimitedStream(stream, max_content_length, is_max=True),
            )
...
class LimitedStream(io.RawIOBase):
    ...
    def __init__(
        self, stream: t.IO[bytes], limit: int, is_max: bool = False
    ) -> None:
        self._stream = stream
        self._pos = 0
        self.limit = limit
