...
for idx, tv in enumerate(test_vectors_verify):
    ...
    public_key = ECC.construct(
        curve=curve_name, point_x=tv.qcavsx, point_y=tv.qcavsy
    )
    ...
    if idx == 1:
        setattr(
            FIPS_ECDH_Tests_KAT,
            "test_verify_positive_rev_%d" % idx,
            ecdh_test_rev,
        )
...
class ECDH_Tests(unittest.TestCase):
    ...
    def test_7(self):
        ...
        kdf = lambda x: SHA256.new(x).digest()
        z = key_agreement(
            kdf=kdf, eph_pub=self.eph_pub, eph_priv=self.eph_priv
        )
        self.assertEqual(
            hexlify(z),
    ...
    def test_8(self):
        ...
        kdf = lambda x: SHA256.new(x).digest()
        z = key_agreement(
            kdf=kdf, static_priv=self.static_priv, eph_pub=self.eph_pub
        )
        self.assertEqual(
            hexlify(z),
    ...
    def test_9(self):
        ...
        kdf = lambda x: SHA256.new(x).digest()
        z = key_agreement(
            kdf=kdf, static_pub=self.static_pub, eph_priv=self.eph_priv
        )
        self.assertEqual(
            hexlify(z),
...
class X25519_Tests(unittest.TestCase):
    ...
    def test_weak(self):
        ...
        for x in weak_keys:
            self.assertRaises(
                ValueError, DH.import_x25519_public_key, unhexlify(x)
            )
...
class X448_Tests(unittest.TestCase):
    ...
    def test_weak(self):
        ...
        for x in weak_keys:
            self.assertRaises(
                ValueError, DH.import_x448_public_key, unhexlify(x)
            )
