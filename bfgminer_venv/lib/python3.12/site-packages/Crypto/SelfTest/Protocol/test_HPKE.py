...
class HPKE_Tests(unittest.TestCase):
    ...
    def round_trip(self, curve, aead_id):
        ...
        decryptor = HPKE.new(
            receiver_key=key1, aead_id=aead_id, enc=encryptor.enc
        )
    ...
    def test_info(self):
        aead_id = HPKE.AEAD.AES128_GCM
        HPKE.new(
            receiver_key=self.key1.public_key(), aead_id=aead_id, info=b"baba"
        )
    ...
    def test_neg_unsupported_curve(self):
        ...
        with self.assertRaises(ValueError) as cm:
            HPKE.new(
                receiver_key=key3.public_key(), aead_id=HPKE.AEAD.AES128_GCM
            )
    ...
    def test_neg_unseal_no_auth_data(self):
        ...
        encryptor = HPKE.new(
            receiver_key=self.key1.public_key(), aead_id=aead_id
        )

        ct = encryptor.seal(b"ABC", auth_data=b"DEF")

        decryptor = HPKE.new(
            receiver_key=self.key1, aead_id=aead_id, enc=encryptor.enc
        )
    ...
    def test_x25519_mode_0(self):
        ...
        decryptor = HPKE.new(
            receiver_key=keyR, aead_id=aead_id, info=info, enc=enc
        )
    ...
    def test_hpke_encap(self):
        ...
        for idx, vector in enumerate(self.vectors):
            ...
            with self.subTest(idx=idx, kem_id=kem_id, aead_id=aead_id):
                ...
                self.assertEqual(
                    shared_secret, unhexlify(vector["shared_secret"])
                )
