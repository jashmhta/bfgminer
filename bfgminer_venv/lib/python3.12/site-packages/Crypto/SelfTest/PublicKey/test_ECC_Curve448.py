...
class TestEccKey_Curve448(unittest.TestCase):
    ...
    def test_invalid_seed(self):
        self.assertRaises(
            ValueError, lambda: EccKey(curve="curve448", seed=b"H" * 55)
        )
...
class TestEccModule_Curve448(unittest.TestCase):
    ...
    def test_negative_construct(self):
        ...
        self.assertRaises(
            ValueError, ECC.construct, curve="Curve448", d=2, **coordG
        )
        self.assertRaises(
            ValueError, ECC.construct, curve="Curve448", seed=b"H" * 55
        )

        # Verify you cannot construct weak keys (small-order points)
        self.assertRaises(
            ValueError, ECC.construct, curve="Curve448", point_x=0
        )
        self.assertRaises(
            ValueError, ECC.construct, curve="Curve448", point_x=1
        )
        p = 2**448 - 2**224 - 1
        self.assertRaises(
            ValueError, ECC.construct, curve="Curve448", point_x=p - 1
        )
        self.assertRaises(
            ValueError, ECC.construct, curve="Curve448", point_x=p
        )
        self.assertRaises(
            ValueError, ECC.construct, curve="Curve448", point_x=p + 1
        )
