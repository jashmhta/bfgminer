...
class DerObject(object):
    ...
    def __init__(
        ...
        if implicit is not None:
            ...
            self._tag_octet = (
                0x80 | constructed_bit | self._convertTag(implicit)
            )
    ...
    def _decodeFromStream(self, s, strict):
        ...
        if hasattr(self, "_inner_tag_octet"):
            ...
            if p.remaining_data() > 0:
                raise ValueError(
                    "Unexpected extra data after the DER structure"
                )
    ...
    def _decodeFromStream(self, s, strict):
        ...
        if strict:
            if len(self.payload) == 0:
                raise ValueError(
                    "Invalid encoding for DER INTEGER: empty payload"
                )
            if (
                len(self.payload) >= 2
                ...
                raise ValueError(
                    "Invalid encoding for DER INTEGER: leading zero"
                )
...
class DerBoolean(DerObject):
    ...
    def _decodeFromStream(self, s, strict):
        ...
        if len(self.payload) != 1:
            raise ValueError(
                "Invalid encoding for DER BOOLEAN: payload is not 1 byte"
            )
...
class DerSequence(DerObject):
    ...
    def _decodeFromStream(self, s, strict):
        ...
        if not ok:
            raise ValueError(
                "Unexpected number of members (%d)"
        ...
        while p.remaining_data() > 0:
            ...
            if setIdOctet < 0:
                ...
                if setIdOctet != der._tag_octet:
                    raise ValueError(
                        "Not all elements are of the same DER type"
                    )
