...
def PBKDF1(password, salt, dkLen, count=1000, hashAlgo=None):
    ...
    if len(salt) != 8:
        raise ValueError(
            "Salt is not 8 bytes long (%d bytes instead)." % len(salt)
        )
...
def PBKDF2(
    password, salt, dkLen=16, count=1000, prf=None, hmac_hash_module=None
):
    ...
    if prf or not hasattr(hmac_hash_module, "_pbkdf2_hmac_assist"):
        ...
        while len(key) < dkLen:
            ...
            first_digest = (
                base.copy().update(salt + struct.pack(">I", i)).digest()
            )
...
def _HKDF_expand(prk, info, L, hashmod):
    ...
    while tlen < L:
        hmac = HMAC.new(
            prk, t[-1] + info + struct.pack("B", n), digestmod=hashmod
        )
...
def HKDF(master, key_len, salt, hashmod, num_keys=1, context=None):
    ...
    if num_keys == 1:
        return okm[:key_len]
    kol = [
        okm[idx : idx + key_len] for idx in iter_range(0, output_len, key_len)
    ]
...
def _bcrypt_hash(password, cost, salt, constant, invert):
    ...
    cipher = _EKSBlowfish.new(
        password, _EKSBlowfish.MODE_ECB, salt, cost, invert
    )
    ctext = constant
    for _ in range(64):
...
def bcrypt(password, cost, salt=None):
    ...
    ctext = _bcrypt_hash(
        password, cost, salt, b"OrpheanBeholderScryDoubt", True
    )
...
def bcrypt_check(password, bcrypt_hash):
    ...
    p = re.compile(
        rb"\$2a\$([0-9][0-9])\$([A-Za-z0-9./]{22,22})([A-Za-z0-9./]{31,31})"
    )
...
def SP800_108_Counter(
    master, key_len, prf, num_keys=None, label=b"", context=b""
):
    """Derive one or more keys from a master secret using
