...
class HPKE_Cipher:
    ...
    def __init__(
        ...
        try:
            self._kem_id, self._kdf_id, self._hashmod = _Curve_Config[
                self._curve
            ]
        ...
        if self._encrypt:
            ...
            if enc is not None:
                raise ValueError(
                    "Parameter 'enc' cannot be an input  when sealing"
                )
    ...
    def _encap(
        ...
        def kdf(
            dh, kem_context=kem_context, suite_id=suite_id, hashmod=hashmod
        ):
    ...
    def _decap(
        ...
        try:
            ...
            raise DeserializeError(
                "'enc' is not a valid encapsulated HPKE key"
            ) from ve
    ...
    def _key_schedule(
        ...
        info_hash = _labeled_extract(
            b"", b"info_hash", info, suite_id, self._hashmod
        )

        key_schedule_context = (
            self._mode.to_bytes(1, "big") + psk_id_hash + info_hash
        )
    ...
    def _new_cipher(self):
        nonce = strxor(
            self._base_nonce, self._sequence.to_bytes(self._Nn, "big")
        )
        if self._aead_id in (AEAD.AES128_GCM, AEAD.AES256_GCM):
            cipher = AES.new(
                self._key, AES.MODE_GCM, nonce=nonce, mac_len=self._Nt
            )
