...
class _Curves(object):
    ...
    def load(self, name):
        if name in self.p192_names:
            ...
            self.curves.update(
                dict.fromkeys(self.curve25519_names, curve25519)
            )
    ...
    def __getitem__(self, name):
        with self.curves_lock:
            ...
            if curve is None:
                ...
                if (
                    name in self.curve25519_names
                    or name in self.curve448_names
                ):
                ...
                curve.is_weierstrass = not (
                    curve.is_edwards or curve.is_montgomery
                )
        return curve
    ...
    * In-place multiplication by a scalar: ``T *= k``
    ...
    def __init__(self, x, y, curve="p256"):
        ...
        if result:
            ...
            raise ValueError(
                "Error %d while instantiating an EC point" % result
            )
    ...
    * In-place multiplication by a scalar: ``T *= k``
    ...
    def __init__(self, x, curve):
        ...
        if self._curve.id not in (CurveID.CURVE25519, CurveID.CURVE448):
            raise ValueError(
                "EccXPoint can only be created for Curve25519/Curve448"
            )
    ...
    def x(self):
        ...
        result = get_x(
            c_uint8_ptr(xb), c_size_t(modulus_bytes), self._point.get()
        )
        if result == 19:  # ERR_ECC_PAI
            raise ValueError("No X coordinate for the point at infinity")
        if result:
            raise ValueError(
                "Error %d while getting X of an EC point" % result
            )
