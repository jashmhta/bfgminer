...
class DsaKey(object):
    ...
    def __init__(self, key_dict):
        ...
        if not public_set.issubset(input_set):
            raise ValueError(
                "Some DSA components are missing = %s"
    ...
    def public_key(self):
        ...
        public_components = dict(
            (k, self._key[k]) for k in ("y", "g", "p", "q")
        )
    ...
    def export_key(
        ...
        if format == "PEM":
            pem_str = PEM.encode(
                binary_key, key_type + " KEY", passphrase, randfunc
            )
...
def _generate_domain(L, randfunc):
    ...
    while True:
        ...
        if (
            p.size_in_bits() == L
            and test_probable_prime(p, randfunc) == PROBABLY_PRIME
        ):
            break
        offset += n + 1
...
def generate(bits, randfunc=None, domain=None):
    ...
    if (L, N) not in [(1024, 160), (2048, 224), (2048, 256), (3072, 256)]:
        raise ValueError(
            "Lengths of p and q (%d, %d) are not compatible"
            "to FIPS 186-3" % (L, N)
        )
