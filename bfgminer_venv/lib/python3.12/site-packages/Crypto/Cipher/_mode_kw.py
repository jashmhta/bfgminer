...
def W_inverse(
    cipher: ModuleType, ciphertext: Union[bytes, bytearray]
) -> bytes:
...
class KWMode(object):
    ...
    def seal(self, plaintext: Union[bytes, bytearray]) -> bytes:
        ...
        if len(plaintext) % 8:
            raise ValueError(
                "The plaintext must have length multiple of 8 bytes"
            )
    ...
    def unseal(self, ciphertext: Union[bytes, bytearray]) -> bytes:
        ...
        if len(ciphertext) % 8:
            raise ValueError(
                "The ciphertext must have length multiple of 8 bytes"
            )
