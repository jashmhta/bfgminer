...
class TestDict(object):
    ...
    def test_merge_iterable_arg(self):
        D, kw = self.D, self.kw
        assert merge([D({1: 1, 2: 2}), D({3: 4})], **kw) == D(
            {1: 1, 2: 2, 3: 4}
        )
    ...
    def test_merge_with(self):
        ...
        assert merge_with(tuple, *dicts, **kw) == D(
            {1: (1, 10), 2: (2, 20), 3: (3,)}
        )
    ...
    def test_itemfilter(self):
        D, kw = self.D, self.kw
        assert itemfilter(
            lambda item: iseven(item[0]), D({1: 2, 2: 3}), **kw
        ) == D({2: 3})
        assert itemfilter(
            lambda item: iseven(item[1]), D({1: 2, 2: 3}), **kw
        ) == D({1: 2})
    ...
    def test_update_in(self):
        ...
        assert update_in(D({}), [0, 1], inc, default=-1, **kw) == D(
            {0: D({1: 0})}
        )
        assert update_in(D({}), [0, 1], str, default=100, **kw) == D(
            {0: D({1: "100"})}
        )
        assert update_in(
            D({"foo": "bar", 1: 50}), ["d", 1, 0], str, 20, **kw
    ...
    def test_factory(self):
        ...
        assert raises(
            TypeError, lambda: merge(D({1: 2}), D({2: 3}), factoryy=dict)
        )
