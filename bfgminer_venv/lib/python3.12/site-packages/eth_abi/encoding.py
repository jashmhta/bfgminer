...
class TupleEncoder(BaseEncoder):
    ...
    def encode(self, values):
        ...
        head_length = sum(
            32 if item is None else len(item) for item in raw_head_chunks
        )
...
class FixedSizeEncoder(BaseEncoder):
    ...
    def encode(self, value):
        ...
        if self.is_big_endian:
            padded_encoded_value = zpad(
                base_encoded_value, self.data_byte_size
            )
        else:
            padded_encoded_value = zpad_right(
                base_encoded_value, self.data_byte_size
            )
...
class NumberEncoder(Fixed32ByteSizeEncoder):
    ...
    def validate_value(self, value):
        ...
        illegal_value = (
            self.illegal_value_fn is not None and self.illegal_value_fn(value)
        )
        if illegal_value:
...
class UnsignedFixedEncoder(BaseFixedEncoder):
    def bounds_fn(self, value_bit_size):
        return compute_unsigned_fixed_bounds(
            self.value_bit_size, self.frac_places
        )
...
class SignedFixedEncoder(BaseFixedEncoder):
    def bounds_fn(self, value_bit_size):
        return compute_signed_fixed_bounds(
            self.value_bit_size, self.frac_places
        )
