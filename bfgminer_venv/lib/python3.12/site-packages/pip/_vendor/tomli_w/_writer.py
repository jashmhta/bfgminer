...
def gen_table_chunks(
    ...
    *,
    ...
    for k, v in table.items():
        ...
        elif is_aot(v) and not all(
            is_suitable_inline_table(t, ctx) for t in v
        ):
    ...
    for k, v, in_aot in tables:
        ...
        yield from gen_table_chunks(
            v, ctx, name=display_name, inside_aot=in_aot
        )
...
def format_inline_array(
    obj: tuple | list, ctx: Context, nest_level: int
) -> str:
    if not obj:
        return "[]"
...
def is_suitable_inline_table(obj: Mapping, ctx: Context) -> bool:
    ...
    return (
        len(rendered_inline) <= MAX_LINE_LENGTH and "\n" not in rendered_inline
    )
