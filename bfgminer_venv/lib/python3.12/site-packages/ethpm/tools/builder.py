...
def normalize_bytecode_object(
    ...
    if link_references:
        yield "linkReferences", process_link_references(
...
@curry
def replace_link_ref_in_bytecode(
    offset: int, length: int, bytecode: str
) -> str:
    new_bytes = (
        bytecode[:offset]
        + "0" * length
...
def normalize_link_ref(
    link_ref: Dict[str, Any], bytecode: str
) -> Dict[str, Any]:
...
def normalize_offsets(
    data: Dict[str, Any], bytecode: str
) -> Iterable[List[int]]:
...
def _deployment(
    ...
    if block:
        if not is_string(block) and not is_hex(block):
            raise ManifestBuildingError(
                f"Block hash: {block} is not a valid hexstring"
            )
    ...
    return assoc_in(
        manifest,
        ["deployments", block_uri, contract_instance],
        deployment_data,
    )
...
def _build_dependency(
    package_name: str, uri: URI, manifest: Manifest
) -> Manifest:
...
def init_manifest(
    package_name: str,
    version: str,
    manifest_version: Optional[str] = "ethpm/3",
) -> Dict[str, Any]:
    """
