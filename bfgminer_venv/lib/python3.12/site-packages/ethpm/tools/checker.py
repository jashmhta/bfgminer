...
@curry
def check_authors(
    meta: Dict[str, Any], warnings: Dict[str, str]
) -> Dict[str, str]:
...
def check_description(
    ...
    if "description" not in meta or not meta["description"]:
        return assoc(
            warnings, "meta.description", WARNINGS["description_missing"]
        )
    return warnings
...
def check_contract_types(
    ...
    if "contractTypes" not in manifest or not manifest["contractTypes"]:
        return assoc(
            warnings, "contractTypes", WARNINGS["contract_type_missing"]
        )
...
def check_deployment_bytecode(
    ...
    return build(
        ...
        check_bytecode_object(
            contract_name, "deployment", data["deploymentBytecode"]
        ),
    )
...
def check_runtime_bytecode(
    ...
    return build(
        warnings,
        check_bytecode_object(
            contract_name, "runtime", data["runtimeBytecode"]
        ),
    )
...
def check_bytecode_object(
    ...
    if "bytecode" not in bytecode_data or not bytecode_data["bytecode"]:
        return assoc_in(
            ...
            WARNINGS["bytecode_subfield_missing"].format(
                contract_name, bytecode_type
            ),
        )
    return warnings
