...
class LinkableContract(Contract):
    ...
    def factory(
        ...
        if dep_link_refs and bytecode:
            if not is_prelinked_bytecode(
                to_bytes(hexstr=bytecode), dep_link_refs
            ):
                needs_bytecode_linking = True
        kwargs = assoc(
            kwargs, "needs_bytecode_linking", needs_bytecode_linking
        )
    ...
    def link_bytecode(
        ...
        bytecode = apply_all_link_refs(
            cls.bytecode, cls.unlinked_references, attr_dict
        )
...
def is_prelinked_bytecode(
    bytecode: bytes, link_refs: List[Dict[str, Any]]
) -> bool:
    """
...
def apply_link_ref(
    ...
    try:
        ...
        raise BytecodeLinkingError(
            "Link references cannot be applied to bytecode"
        )

    address = (
        value if is_canonical_address(value) else to_canonical_address(value)
    )
    new_bytes = (
