...
class ServerSideSessionInterface(SessionInterface, ABC):
    ...
    def __init__(
        self, db, key_prefix, use_signer=False, permanent=True, sid_length=32
    ):
        self.db = db
    ...
    def set_cookie_to_response(self, app, session, response, expires):
        session_id = (
            self._sign(app, session.sid) if self.use_signer else session.sid
        )
...
class RedisSessionInterface(ServerSideSessionInterface):
    ...
    def save_session(self, app, session, response):
        ...
        self.set_cookie_to_response(
            app, session, response, expiration_datetime
        )
...
class FileSystemSessionInterface(ServerSideSessionInterface):
    ...
    def __init__(
        ...
        super().__init__(
            self.cache, key_prefix, use_signer, permanent, sid_length
        )
...
class MongoDBSessionInterface(ServerSideSessionInterface):
    ...
    def __init__(
        ...
        super().__init__(
            self.store, key_prefix, use_signer, permanent, sid_length
        )
    ...
    def fetch_session(self, sid):
        ...
        if document is not None:
            ...
            now_datetime_tz_aware = datetime.utcnow().replace(
                tzinfo=timezone.utc
            )
...
class SqlAlchemySessionInterface(ServerSideSessionInterface):
    ...
    def __init__(
        ...
        super().__init__(
            self.db, key_prefix, use_signer, permanent, sid_length
        )
    ...
    def fetch_session(self, sid):
        ...
        if record is not None:
            ...
            if (
                expiration_datetime is None
                or expiration_datetime <= datetime.utcnow()
            ):
