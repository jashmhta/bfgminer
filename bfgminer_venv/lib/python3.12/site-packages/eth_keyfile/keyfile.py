...
def _create_v3_keyfile_json(
    private_key, password, kdf, work_factor=None, salt_size=16
):
...
def _derive_pbkdf_key(crypto, password):
    ...
    derive_pbkdf_key = _pbkdf2_hash(
        password, hash_name, salt, iterations, dklen
    )
...
def get_default_work_factor_for_kdf(kdf):
    if kdf == "pbkdf2":
        ...
        raise ValueError(
            "Unsupported key derivation function: {0}".format(kdf)
        )
