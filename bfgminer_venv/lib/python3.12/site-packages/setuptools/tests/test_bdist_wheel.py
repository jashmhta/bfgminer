...
def test_licenses_deprecated(dummy_dist, monkeypatch, tmp_path):
    ...
    with ZipFile("dist/dummy_dist-1.0-py3-none-any.whl") as wf:
        license_files = {
            "dummy_dist-1.0.dist-info/licenses/licenses_dir/DUMMYFILE"
        }
        assert set(wf.namelist()) == DEFAULT_FILES | license_files
...
def test_licenses_preserve_folder_structure(
    licenses_dist, monkeypatch, tmp_path
):
    ...
    with ZipFile("dist/licenses_dist-1.0-py3-none-any.whl") as wf:
        default_files = {
            name.replace("dummy_", "licenses_") for name in DEFAULT_FILES
        }
        license_files = {
        ...
        assert set(wf.namelist()) == default_files | license_files
        metadata = wf.read("licenses_dist-1.0.dist-info/METADATA").decode(
            "utf8"
        )
