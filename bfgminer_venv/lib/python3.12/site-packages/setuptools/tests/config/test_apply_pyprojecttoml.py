...
def _mock_expand_patterns(patterns, *_, **__):
    ...
    """
    return [
        re.sub("[^a-z0-9]+", "", p, flags=re.I) or "empty" for p in patterns
    ]
...
def test_apply_pyproject_equivalent_to_setupcfg(url, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        Distribution,
        "_expand_patterns",
        Mock(side_effect=_mock_expand_patterns),
    )
...
class TestLicenseFiles:
    def base_pyproject(
        ...
        text = f"{text}\n{additional_text}\n"
        pyproject = _pep621_example_project(
            tmp_path, "README", pyproject_text=text
        )
        return pyproject
    ...
    def test_missing_patterns(self, tmp_path):
        ...
        msg2 = (
            "Normalizing 'licenseref-Proprietary' to 'LicenseRef-Proprietary'"
        )
        with (
            pytest.warns(SetuptoolsDeprecationWarning, match=msg1),
    ...
    def test_deprecated_file_expands_to_text(self, tmp_path):
        ...
        assert (
            'license-files = ["LICENSE.txt"]' in PEP621_EXAMPLE
        )  # sanity check
        ...
        [
            ...
            pytest.param(
                *("license", "license", "Not SPDX"),
                marks=[
                    pytest.mark.filterwarnings("ignore:.*license. overwritten")
                ],
            ),
