...
(si_sb_C_i_list, si_sb_C_i_bysize, pl_sb_C_i_bysize, pl_sb_C_i) = (
    make_pl_si_lists(pl_sb_C_i_list, "i", None)
)
...
(si_sb_C_im_list, si_sb_C_im_bysize, pl_sb_C_im_bysize, pl_sb_C_im) = (
    make_pl_si_lists(pl_sb_C_im_list, "im", None)
)
...
pl_sb_postfix_adj: Iterable[str] = (
    enclose(val + f"(?=(?:-|\\s+){key})")
    for key, val in _pl_sb_postfix_adj_defn
)
...
class engine:
    ...
    @typechecked
    def ud_match(
        self, word: Word, wordlist: Sequence[Optional[Word]]
    ) -> Optional[str]:
        for i in range(
            len(wordlist) - 2, -2, -2
        ):  # backwards through even elements
    ...
    def plural_verb(
        ...
        plural = self.postprocess(
            word,
            self._pl_special_verb(word, count)
    ...
    def _plequal(
        self, word1: str, word2: str, pl
    ) -> Union[str, bool]:  # noqa: C901
    ...
    def _pl_reg_plurals(
        self, pair: str, stems: str, end1: str, end2: str
    ) -> bool:
    ...
    def _pl_check_plurals_adj(self, word1: str, word2: str) -> bool:
        word1a = (
            word1[: word1.rfind("'")] if word1.endswith(("'s", "'")) else ""
        )
        word2a = (
            word2[: word2.rfind("'")] if word2.endswith(("'s", "'")) else ""
        )
    ...
    def get_count(
        self, count: Optional[Union[str, int]] = None
    ) -> Union[str, int]:
    ...
    def _plnoun(  # noqa: C901
        ...
        if (
            ...
            and word.last.lower() in pl_sb_uninflected_herd
        ):
            return word
    ...
    def _handle_long_compounds(
        ...
        solutions = (  # type: ignore
            ...
            for leader, (cand, prep), trailer in windowed_complete(
                word.split_, 2
            )
    ...
    def _pl_special_verb(  # noqa: C901
        ...
        if (
            ...
            and words.first[:-3] in plverb_irregular_pres
        ):
            return (
    ...
    def _sinoun(  # noqa: C901
        ...
        if (
            self.classical_dict["herd"]
            and words.last.lower() in pl_sb_uninflected_herd
        ):
            return word
        ...
        if mo and mo.group(2) != "":
            return (
                f"{self._sinoun(mo.group(1), 1, gender=gender)}{mo.group(2)}"
            )
        ...
        if self.classical_dict["ancient"]:
            ...
            if words.lowered[-4:] in ("eaux", "ieux"):
                return word[:-1]
            if (
                words.lowered[-5:] in ("ynges", "inges", "anges")
                and len(word) > 6
            ):
                return word[:-3] + "x"
    ...
    def hundfn(self, hundreds: int, tens: int, units: int, mindex: int) -> str:
        if hundreds:
            andword = (
                f" {self._number_args['andword']} " if tens or units else ""
            )
    ...
    def group3sub(self, mo: Match) -> str:
        ...
        if tens:
            ...
            tenword = (
                f" {self._number_args['zero']} {self._number_args['zero']}"
            )
