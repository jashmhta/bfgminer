...
pyd_re = re.compile(
    r"\.(?P<abi>[a-z0-9]+)-(?P<platform>win32|win_amd64)\.pyd$"
)
    ...
    for pat in files:
        for archive in iglob(pat):
            ...
            with WheelFile(dest_path, "w") as wheelfile:
                ...
                tags = parse_tag(
                    f"{source.pyver}-{source.abi}-{source.platform}"
                )
