...
def _AddEnumValues(descriptor, cls):
    ...
    for enum_type in descriptor.enum_types:
        setattr(
            cls, enum_type.name, enum_type_wrapper.EnumTypeWrapper(enum_type)
        )
...
def _GetInitializeDefaultForMap(field):
    if not field.is_repeated:
        raise ValueError(
            "map_entry set on non-repeated field %s" % (field.name)
        )
...
def _ReraiseTypeErrorWithFieldName(message_name, field_name):
    ...
    if len(exc.args) == 1 and type(exc) is TypeError:
        ...
        exc = TypeError(
            "%s for field %s.%s" % (str(exc), message_name, field_name)
        )
...
def _AddListFieldsMethod(message_descriptor, cls):
    ...
    def ListFields(self):
        all_fields = [
            item for item in self._fields.items() if _IsPresent(item)
        ]
        ...
        while pos != end:
            ...
            if field_des is None:
                ...
                self._unknown_fields.append(
                    (
                        tag_bytes,
                        buffer[pos + len(tag_bytes) : new_pos].tobytes(),
                    )
                )
                pos = new_pos
            else:
