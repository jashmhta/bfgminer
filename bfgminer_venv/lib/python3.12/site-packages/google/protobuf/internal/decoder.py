...
def GroupDecoder(field_number, is_repeated, is_packed, key, new_default):
    ...
    if is_repeated:
        tag_bytes = encoder.TagBytes(
            field_number, wire_format.WIRETYPE_START_GROUP
        )
        tag_len = len(tag_bytes)
        ...
        def DecodeRepeatedField(
            ...
            while 1:
                ...
                pos = value.add()._InternalParse(
                    buffer, pos, end, current_depth
                )
                current_depth -= 1
                # Read end tag.
    ...
    if is_repeated:
        ...
        def DecodeRepeatedField(
            ...
            while 1:
                ...
                if (
                    value.add()._InternalParse(
                        buffer, pos, new_pos, current_depth
                    )
                    != new_pos
                ):
        ...
        def DecodeField(
            ...
            if (
                value._InternalParse(buffer, pos, new_pos, current_depth)
                != new_pos
            ):
...
def MessageSetItemDecoder(descriptor):
    ...
    def DecodeItem(buffer, pos, end, message, field_dict):
        ...
        while 1:
            ...
            if tag_bytes == type_id_tag_bytes:
                ...
                _, pos = _DecodeUnknownField(
                    buffer, pos, end, field_number, wire_type
                )
                if pos == -1:
                    raise _DecodeError("Unexpected end-group tag.")
        ...
        if extension is not None:
            ...
            if (
                value._InternalParse(buffer, message_start, message_end)
                != message_end
            ):
                # The only reason _InternalParse would return early is if it encountered
                # an end-group tag.
