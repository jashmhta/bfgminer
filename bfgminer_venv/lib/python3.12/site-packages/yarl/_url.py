...
class URL:
    ...
    def __new__(
        ...
        *,
        ...
        if isinstance(val, str):
            return (
                pre_encoded_url(str(val)) if encoded else encode_url(str(val))
            )
        if val is UNDEFINED:
    ...
    def build(
        ...
        *,
        ...
        if port is not None and not isinstance(port, int):
            raise TypeError(
                f"The port is required to be int, got {type(port)!r}."
            )
    ...
    def __str__(self) -> str:
        ...
        ) is not None and port == DEFAULT_PORTS.get(self._scheme):
    ...
    def __bool__(self) -> bool:
        return bool(
            self._netloc or self._path or self._query or self._fragment
        )
    ...
    def _cache_netloc(self) -> None:
        ...
        c["raw_user"], c["raw_password"], c["raw_host"], c["explicit_port"] = (
            split_loc
        )
    ...
    def path_qs(self) -> str:
        ...
        return (
            self.path if not (q := self.query_string) else f"{self.path}?{q}"
        )
    ...
    def raw_path_qs(self) -> str:
        ...
        if q := self._query:
            return (
                f"{self._path}?{q}"
                if self._path or not self._netloc
                else f"/?{q}"
            )
        return self._path if self._path or not self._netloc else "/"
    ...
    def _make_child(
        ...
        for idx, path in enumerate(reversed(paths)):
            ...
            parsed += (
                segments[1:] if not last and segments[0] == "" else segments
            )
    ...
    def with_name(
        ...
        *,
        ...
        fragment = self._fragment if keep_fragment else ""
        return from_parts(
            self._scheme, netloc, "/".join(parts), query, fragment
        )
    ...
    def human_repr(self) -> str:
        ...
        return unsplit_result(
            self._scheme, netloc, path, query_string, fragment
        )
