...
def test_apply():
    ...
    assert tuple(map(apply, [double, inc, double], [10, 500, 8000])) == (
        20,
        501,
        16000,
    )
...
def test_memoize_key_signature():
    ...
    mf = memoize(
        lambda x=0: False,
        cache={(None, frozenset((("x", 1),))): 1, ((1,), None): 2},
    )
    assert mf() is False
    assert mf(x=1) == 1
    assert mf(1) == 2
...
def test_compose_metadata():
    ...
    if not IS_PYPY_GE_39:  # pypy >=3.9 doesn't like __doc__ property
        assert (
            composed.__doc__ == "lambda *args, **kwargs: f(g(*args, **kwargs))"
        )
...
def test_compose_left():
    for (
        ...
        kw,
        expected,
    ) in generate_compose_left_test_cases():
...
def test_excepts():
    ...
    testlines = "\n".join(
        (line.strip() for line in excepts.__doc__.splitlines())
    )
    assert (
