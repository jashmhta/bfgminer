...
def test_dicttoolz():
    ...
    # XXX
    assert (
        raises(TypeError, lambda: get_in(None, {})) or get_in(None, {}) is None
    )
    ...
    assert raises(
        (AttributeError, TypeError), lambda: keyfilter(identity, None)
    )
    tested.append("keyfilter")

    # XXX
    assert raises(TypeError, lambda: keymap(None, {1: 2})) or keymap(
        None, {1: 2}
    ) == {(1,): 2}
    ...
    assert raises(
        (TypeError, AttributeError), lambda: merge_with(identity, None, None)
    )
    ...
    assert raises(
        (AttributeError, TypeError), lambda: valfilter(identity, None)
    )
    tested.append("valfilter")

    # XXX
    assert raises(TypeError, lambda: valmap(None, {1: 2})) or valmap(
        None, {1: 2}
    ) == {1: (2,)}
    ...
    assert raises(
        (AttributeError, TypeError), lambda: itemfilter(identity, None)
    )
    tested.append("itemfilter")

    assert raises(
        (AttributeError, TypeError), lambda: assoc_in(None, [2, 2], 3)
    )
...
def test_itertoolz():
    ...
    assert raises(
        ...
        lambda: list(
            interleave(
                [[1, 2], GenException(ValueError)], pass_exceptions=None
            )
        ),
    )
    ...
    # XXX
    assert raises(
        TypeError, lambda: list(take(None, [1, 2])) == [1, 2]
    ) or list(take(None, [1, 2])) == [1, 2]
    assert raises(TypeError, lambda: list(take(1, None)))
    tested.append("take")

    # XXX
    assert raises(
        TypeError, lambda: list(tail(None, [1, 2])) == [1, 2]
    ) or list(tail(None, [1, 2])) == [1, 2]
