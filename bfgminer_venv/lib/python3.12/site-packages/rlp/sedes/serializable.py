...
def validate_args_and_kwargs(args, kwargs, arg_names, allow_missing=False):
    ...
    if duplicate_kwargs:
        raise TypeError(
            "Duplicate kwargs: {0}".format(sorted(duplicate_kwargs))
        )
...
def merge_kwargs_to_args(args, kwargs, arg_names, allow_missing=False):
    validate_args_and_kwargs(
        args, kwargs, arg_names, allow_missing=allow_missing
    )
...
def _eq(left, right):
    ...
    if isinstance(left, (tuple, list)) and isinstance(right, (tuple, list)):
        return len(left) == len(right) and all(
            _eq(*pair) for pair in zip(left, right)
        )
    else:
        return left == right
...
class BaseChangeset:
    ...
    def build_rlp(self):
        if self.__state__ == ChangesetState.OPEN:
            ...
            raise ValueError(
                "Cannot open Changeset which is not in the OPEN state"
            )
    ...
    def close(self):
        if self.__state__ == ChangesetState.OPEN:
            ...
            raise ValueError(
                "Cannot close Changeset which is not in the OPEN state"
            )
...
class BaseSerializable(collections.abc.Sequence):
    def __init__(self, *args, **kwargs):
        if kwargs:
            field_values = merge_kwargs_to_args(
                args, kwargs, self._meta.field_names
            )
        else:
            field_values = args
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            raise IndexError(
                "Unsupported type for __getitem__: {0}".format(type(idx))
            )
    ...
    def __repr__(self):
        keyword_args = tuple(
            "{}={!r}".format(k, v) for k, v in self.as_dict().items()
        )
        return "{}({})".format(
            type(self).__name__,
            ", ".join(keyword_args),
    ...
    def serialize(cls, obj):
        try:
            ...
            raise ObjectSerializationError(
                obj=obj, sedes=cls, list_exception=e
            )
    ...
    def deserialize(cls, serial, **extra_kwargs):
        try:
            ...
            raise ObjectDeserializationError(
                serial=serial, sedes=cls, list_exception=e
            )

        args_as_kwargs = merge_args_to_kwargs(
            values, {}, cls._meta.field_names
        )
    ...
    def copy(self, *args, **kwargs):
        ...
        all_kwargs = merge_args_to_kwargs(
            args, combined_kwargs, self._meta.field_names
        )
...
class SerializableBase(abc.ABCMeta):
    def __new__(cls, name, bases, attrs):
        ...
        serializable_bases = tuple(
            b for b in bases if isinstance(b, SerializableBase)
        )
