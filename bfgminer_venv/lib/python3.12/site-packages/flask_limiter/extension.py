...
class LimiterContext:
    ...
    view_rate_limits: List[RequestLimit] = dataclasses.field(
        default_factory=list
    )
    conditional_deductions: Dict[Limit, List[str]] = dataclasses.field(
        default_factory=dict
    )
    seen_limits: OrderedSet[Limit] = dataclasses.field(
...
class Limiter:
    ...
    def __init__(
        ...
        *,
        ...
        application_limits_cost: Optional[
    ...
    def init_app(self, app: flask.Flask) -> None:
        ...
        if self._swallow_errors is None:
            self._swallow_errors = bool(
                config.get(ConfigVars.SWALLOW_ERRORS, False)
            )
        ...
        if self._headers_enabled is None:
            self._headers_enabled = bool(
                config.get(ConfigVars.HEADERS_ENABLED, False)
            )

        self._storage_options.update(
            config.get(ConfigVars.STORAGE_OPTIONS, {})
        )
        ...
        self._header_mapping = {
            ...
            HeaderNames.REMAINING: self._header_mapping.get(
                ...
                config.get(
                    ConfigVars.HEADER_REMAINING, HeaderNames.REMAINING.value
                ),
            ),
            ...
            HeaderNames.RETRY_AFTER: self._header_mapping.get(
                ...
                config.get(
                    ConfigVars.HEADER_RETRY_AFTER,
                    HeaderNames.RETRY_AFTER.value,
                ),
            ),
        }
        ...
        self._key_prefix = self._key_prefix or config.get(
            ConfigVars.KEY_PREFIX, ""
        )
        ...
        self._application_limits_cost = (
            self._application_limits_cost
            or config.get(ConfigVars.APPLICATION_LIMITS_COST, 1)
        )
        ...
        self._on_breach = self._on_breach or config.get(
            ConfigVars.ON_BREACH, None
        )
    ...
    def __configure_fallbacks(self, app: flask.Flask, strategy: str) -> None:
        ...
        fallback_enabled = config.get(
            ConfigVars.IN_MEMORY_FALLBACK_ENABLED, False
        )
        ...
        if (
            ...
            try:
                ...
                if existing_retry_after_header is not None:
                    ...
                    retry_after: Optional[Union[float, datetime.datetime]] = (
                        parse_date(existing_retry_after_header)
                    )
    ...
    def __evaluate_limits(self, endpoint: str, limits: List[Limit]) -> None:
            ...
            if not self.limiter.test(lim.limit, *args):
                ...
                if self._on_meta_breach:
                    try:
                        ...
                        if self._swallow_errors:
                            self.logger.exception(
                                "on_meta_breach callback failed with error %s",
                                err,
                            )
                        else:
                            raise err
        ...
        for lim in sorted(limits, key=lambda x: x.limit):
            ...
            request_limit = RequestLimit(
                self, lim.limit, args, False, lim.shared
            )
...
class LimitDecorator:
    ...
    def __call__(
        ...
        if isinstance(obj, flask.Blueprint):
            self.limiter.limit_manager.add_blueprint_limit(
            ...
            def __inner(*a: P.args, **k: P.kwargs) -> R:
                if (
                    ...
                    if identity:
                        ...
                        if (
                            view_func
                            and not get_qualified_name(view_func) == name
                        ):
                            self.limiter.limit_manager.add_endpoint_hint(
                                identity, name
                            )
