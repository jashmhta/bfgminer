...
class LimitManager:
    ...
    def add_decorated_limit(
        ...
        if limit:
            if not override:
                self._decorated_limits.setdefault(route, OrderedSet()).add(
                    limit
                )
            else:
    ...
    def add_blueprint_limit(
        ...
        if limit:
            self._blueprint_limits.setdefault(blueprint, OrderedSet()).add(
                limit
            )
    ...
    def add_blueprint_exemption(
        self, blueprint: str, scope: ExemptionScope
    ) -> None:
    ...
    def resolve_limits(
        ...
        all_limits = (
            ...
            if in_middleware
            and not (exemption_scope & ExemptionScope.APPLICATION)
            else []
        )
    ...
    def exemption_scope(
        ...
        blueprint_instance = (
            app.blueprints.get(blueprint) if blueprint else None
        )
    ...
    def blueprint_limits(
        self, app: flask.Flask, blueprint: str
    ) -> List[Limit]:
        limits: List[Limit] = []
        ...
        if blueprint_instance:
            ...
            blueprint_ancestory = set(
                blueprint.split(".") if blueprint else []
            )

            self_exemption, ancestor_exemptions = (
                self._blueprint_exemption_scope(app, blueprint)
            )
                ...
                blueprint_limits: Iterable[LimitGroup] = (
                    ...
                    if not (
                        ...
                        and all(
                            ...
                            for limit in blueprint_self_limits
                        )
                    )
    ...
    def _blueprint_exemption_scope(
        ...
        exemption = self._blueprint_exemptions[name] & ~(
            ExemptionScope.ANCESTORS
        )
        ...
        return exemption, {
            k: self._blueprint_exemptions[k] for k in ancestor_exemption
        }
