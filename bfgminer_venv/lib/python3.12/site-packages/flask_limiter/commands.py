...
def render_limit_state(
    ...
    if not limiter.storage or (
        limiter.storage and not limiter.storage.check()
    ):
    ...
    for limit in limits[0] + limits[1]:
        if endpoint:
            ...
            source = (
                ...
                if blueprint
                and limit
                in limiter.limit_manager.blueprint_limits(app, blueprint)
                else (
                    "route"
                    if limit
        ...
        if limit.per_method and rule and rule.methods:
            for method in rule.methods:
                ...
                entry = (
                    f"[{source}]{rendered} [http]({method})[/http][/{source}]"
                )
                if test:
    ...
    with current_app.test_request_context():
        ...
        if limiter:
            ...
            extension_details.add_row(
                "Key Function",
                ConfigVars.KEY_FUNC,
                render_func(limiter._key_func),
            )
            extension_details.add_row(
                "Key Prefix",
                ConfigVars.KEY_PREFIX,
                Pretty(limiter._key_prefix),
            )
            ...
            if limiter._headers_enabled:
                ...
                header_values.add(
                    Pretty(limiter._header_mapping[HeaderNames.RESET])
                )
                header_values.add(
...
@click.option(
    "--watch/--no-watch", default=False, help="Create a live dashboard"
)
@with_appcontext  # type: ignore
def limits(
    endpoint: Optional[str] = None,
    ...
    with current_app.test_request_context():
        ...
        if limiter:
            ...
            for rule in sorted(
                current_app.url_map.iter_rules(filter_endpoint),
                key=lambda r: str(r),
            ):
            ...
            def console_renderable() -> Generator:  # type: ignore
                ...
                for name in groups:
                    ...
                    [
                        group_tree.add(renderable())
                        for renderable in groups[name]
                    ]
                    yield group_tree
...
@click.option("--method", default=None, help="HTTP Method to filter by")
@click.option(
    "--key", default=None, required=True, help="Key to reset the limits for"
)
