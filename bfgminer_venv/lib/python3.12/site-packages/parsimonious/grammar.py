...
class Grammar(OrderedDict):
    ...
    * It does all kinds of whizzy space- and time-saving optimizations, like
    ...
    def __init__(self, rules="", **more_rules):
        ...
        exprs, first = self._expressions_from_rules(
    ...
    def __str__(self):
        ...
        exprs.extend(
            expr for expr in self.values() if expr is not self.default_rule
        )
...
class LazyReference(str):
    ...
    def resolve_refs(self, rule_map):
        ...
        while True:
            if cur in seen:
                raise BadGrammar(
                    f"Circular Reference resolving {self.name}={self}."
                )
            else:
                seen.add(cur)
            try:
...
class RuleVisitor(NodeVisitor):
    ...
    def visit_quantified(self, node, quantified):
        ...
        try:
            ...
            if len(quantifier) == 1:
                ...
                max_match = (
                    int(quantifier[1]) if quantifier[1] else float("inf")
                )
    ...
    def visit_spaceless_literal(self, spaceless_literal, visited_children):
        ...
        self._last_literal_node_and_type = spaceless_literal, type(
            literal_value
        )
