...
class FileResponse(StreamResponse):
    ...
    def _seek_and_read(
        self, fobj: IO[Any], offset: int, chunk_size: int
    ) -> bytes:
        fobj.seek(offset)
    ...
    def _make_response(
        ...
        ) is not None and self._etag_match(etag_value, ifnonematch, weak=True):
    ...
    async def prepare(
        ...
        try:
            response_result, fobj, st, file_encoding = (
        ...
        try:
            return await self._prepare_open_file(
                request, fobj, st, file_encoding
            )
        finally:
