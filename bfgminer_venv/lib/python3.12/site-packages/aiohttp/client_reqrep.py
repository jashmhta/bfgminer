...
def _warn_if_unclosed_payload(
    payload: payload.Payload, stacklevel: int = 2
) -> None:
    ...
    def cookies(self) -> SimpleCookie:
        if self._cookies is None:
            if self._raw_cookie_headers is not None:
                ...
                cookies.update(
                    parse_set_cookie_headers(self._raw_cookie_headers)
                )
...
class ClientRequest:
    ...
    __writer: Optional["asyncio.Task[None]"] = (
        None  # async task for streaming data
    )
    ...
    def update_body_from_data(self, body: Any, _stacklevel: int = 3) -> None:
        ...
        try:
            body_payload = payload.PAYLOAD_REGISTRY.get(
                maybe_payload, disposition=None
            )
        ...
        for key, value in body_payload.headers.items():
            if key in headers or (
                skip_headers is not None and key in skip_headers
            ):
                continue
            headers[key] = value
