...
cython_int = (
    int  # Typed to int in Python, but cython with use a signed int in the pxd
)
...
class WebSocketReader:
    ...
    def _handle_frame(
        ...
        opcode: Union[
            int, cython_int
        ],  # Union intended: Cython pxd uses C int
        ...
        if opcode in {OP_CODE_TEXT, OP_CODE_BINARY, OP_CODE_CONTINUATION}:
            ...
            if compressed:
                ...
                if (
                    self._max_msg_size
                    and len(payload_merged) > self._max_msg_size
                ):
                    raise WebSocketError(
                        WSCloseCode.MESSAGE_TOO_BIG,
    ...
    def _feed_data(self, data: bytes) -> None:
        ...
        while True:
            ...
            if self._state == READ_HEADER:
                ...
                if self._frame_fin or self._compressed == COMPRESSED_NOT_SET:
                    self._compressed = (
                        COMPRESSED_TRUE if rsv1 else COMPRESSED_FALSE
                    )
                elif rsv1:
                    raise WebSocketError(
                        WSCloseCode.PROTOCOL_ERROR,
            ...
            if self._state == READ_PAYLOAD_LENGTH:
                ...
                if len_flag == 126:
                    ...
                    if data_len - start_pos < 8:
                        break
                    self._payload_bytes_to_read = UNPACK_LEN3(data, start_pos)[
                        0
                    ]
                    start_pos += 8
                else:
                ...
                self._state = (
                    READ_PAYLOAD_MASK if self._has_mask else READ_PAYLOAD
                )
            ...
            if self._state == READ_PAYLOAD:
                ...
                if self._payload_bytes_to_read != 0:
                    ...
                    self._payload_fragments.append(
                        data_cstr[f_start_pos:f_end_pos]
                    )
                    break
                ...
                if had_fragments:
                    ...
                    if (
                        type(payload_bytearray) is not bytearray
                    ):  # pragma: no branch
        ...
        self._tail = (
            data_cstr[start_pos:data_len] if start_pos < data_len else b""
        )
