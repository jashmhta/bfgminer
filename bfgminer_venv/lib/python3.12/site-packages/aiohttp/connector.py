...
class _TransportPlaceholder:
    ...
    def __init__(
        self, closed_future: asyncio.Future[Optional[Exception]]
    ) -> None:
...
class BaseConnector:
            ...
            for data in self._conns.values():
                for proto, _ in data:
                    if (
                        abort_ssl
                        and proto.transport
                        and proto.transport.get_extra_info("sslcontext")
                        is not None
                    ):
                        proto.abort()
                    else:
    ...
    def _available_connections(self, key: "ConnectionKey") -> int:
        ...
        if (
            self._limit
            and (total_remain := self._limit - len(self._acquired)) <= 0
        ):
            return total_remain
    ...
    async def connect(
        ...
        async with ceil_timeout(timeout.connect, timeout.ceil_threshold):
            ...
            placeholder = cast(
                ResponseHandler,
                _TransportPlaceholder(self._placeholder_future),
            )
...
class TCPConnector(BaseConnector):
    ...
    async def close(self, *, abort_ssl: bool = False) -> None:
        ...
        await super().close(
            abort_ssl=abort_ssl or self._ssl_shutdown_timeout == 0
        )
