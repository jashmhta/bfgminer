...
SUPPORTED_ALGORITHMS: Final[Tuple[str, ...]] = tuple(
    sorted(DigestFunctions.keys())
)
...
def parse_header_pairs(header: str) -> Dict[str, str]:
    ...
    return {
        stripped_key: (
            unescape_quotes(quoted_val) if quoted_val else unquoted_val
        )
        for key, quoted_val, unquoted_val in _HEADER_PAIRS_PATTERN.findall(
            header
        )
...
class DigestAuthMiddleware:
    ...
    async def _encode(
        ...
        if qop == "auth-int":
            if isinstance(
                body, Payload
            ):  # will always be empty bytes unless Payload
