...
class PlPgsqlLexer(PostgresBase, RegexLexer):
    ...
    for i, pattern in enumerate(tokens["root"]):
        if pattern[1] == Keyword:
            tokens["root"][i] = (
                words(
                    _postgres_builtins.KEYWORDS
                    + _postgres_builtins.PLPGSQL_KEYWORDS,
                    suffix=r"\b",
                ),
                Keyword,
            )
...
class PostgresExplainLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (
                words(_postgres_builtins.EXPLAIN_KEYWORDS, suffix=r"\b"),
                Keyword,
            ),
            # join keywords
...
class MySqlLexer(RegexLexer):
    ...
    tokens = {
        "root": [
            ...
            (
                words(
                    _mysql_builtins.MYSQL_CONSTANTS, prefix=r"\b", suffix=r"\b"
                ),
                Name.Constant,
            ),
            (
                words(
                    _mysql_builtins.MYSQL_DATATYPES, prefix=r"\b", suffix=r"\b"
                ),
                Keyword.Type,
            ),
            (
                words(
                    _mysql_builtins.MYSQL_KEYWORDS, prefix=r"\b", suffix=r"\b"
                ),
                Keyword,
            ),
            (
