...
class AsyncJSONBaseProvider(AsyncBaseProvider):
    ...
    def encode_rpc_request(self, method: RPCEndpoint, params: Any) -> bytes:
        ...
        encoded = FriendlyJsonSerde().json_encode(
            rpc_dict, cls=Web3JsonEncoder
        )
        return to_bytes(text=encoded)

    def decode_rpc_response(self, raw_response: bytes) -> RPCResponse:
        text_response = str(
            to_text(raw_response)
            if not is_text(raw_response)
            else raw_response
        )
        return cast(
            RPCResponse, FriendlyJsonSerde().json_decode(text_response)
        )
    ...
    async def is_connected(self, show_traceback: bool = False) -> bool:
        ...
        if response["jsonrpc"] == "2.0":
            ...
            if show_traceback:
                raise ProviderConnectionError(
                    f"Bad jsonrpc version: {response}"
                )
            return False
