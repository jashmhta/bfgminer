...
def type_aware_apply_formatters_to_dict(
    ...
    """
    formatted_dict: Dict[str, Any] = apply_formatters_to_dict(
        formatters, dict(value)
    )
    return (
...
TRANSACTION_RESULT_FORMATTERS = {
    ...
    "r": apply_formatter_if(
        is_not_null, to_hexbytes(32, variable_length=True)
    ),
    "raw": HexBytes,
    "s": apply_formatter_if(
        is_not_null, to_hexbytes(32, variable_length=True)
    ),
...
BLOCK_FORMATTERS = {
    ...
    "nonce": apply_formatter_if(
        is_not_null, to_hexbytes(8, variable_length=True)
    ),
...
FEE_HISTORY_FORMATTERS = {
    ...
    "gasUsedRatio": apply_formatter_if(
        is_not_null, apply_formatter_to_array(float)
    ),
...
PYTHONIC_REQUEST_FORMATTERS: Dict[RPCEndpoint, Callable[..., Any]] = {
    ...
    RPC.eth_getTransactionCount: apply_formatter_at_index(
        to_hex_if_integer, 1
    ),
    ...
    RPC.eth_sendTransaction: apply_formatter_at_index(
        transaction_param_formatter, 0
    ),
    RPC.eth_signTransaction: apply_formatter_at_index(
        transaction_param_formatter, 0
    ),
    ...
    # tracing
    RPC.trace_replayBlockTransactions: apply_formatter_at_index(
        to_hex_if_integer, 0
    ),
...
# -- eth_subscribe -- #
def subscription_formatter(
    value: Any,
) -> Union[HexBytes, HexStr, Dict[str, Any]]:
    if is_hexstr(value):
...
PYTHONIC_RESULT_FORMATTERS: Dict[RPCEndpoint, Callable[..., Any]] = {
    ...
    RPC.eth_signTransaction: apply_formatter_if(
        is_not_null, signed_tx_formatter
    ),
    ...
    RPC.personal_listAccounts: apply_list_to_array_formatter(
        to_checksum_address
    ),
    RPC.personal_listWallets: apply_list_to_array_formatter(
        geth_wallets_formatter
    ),
