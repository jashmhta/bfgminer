...
LATEST_AVAILABLE_SOLIDITY_VERSION = sorted(
...
def _get_compiled_contract_data(
    ...
    if not contract_data:
        raise Exception(
            f"Could not find compiled data for contract: {contract_name}"
        )
...
def compile_files(file_list: List[str]) -> None:
    for filename in file_list:
        ...
        for line in dot_sol_file:
            if (
                all(_ in line for _ in ["contract", "{"])
    ...
    for dot_sol_filename in contracts_in_file.keys():
        ...
        split_and_lowercase = [
            i.lower()
            for i in re.split(r"([A-Z][a-z]*)", filename_no_extension)
            if i
        ]
        python_filename = f"{'_'.join(split_and_lowercase)}.py"
        python_file_path = os.path.join(
            os.getcwd(), "contract_data", python_filename
        )
        try:
        ...
        with open(python_file_path, "w") as f:
            ...
            for c in contracts_in_file[dot_sol_filename]:
                ...
                contract_source = f"# source: web3/_utils/contract_sources/{dot_sol_filename}:{c}"
