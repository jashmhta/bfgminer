        ...
        if is_list_like(arg):
            ...
            for nested in arg:
                if is_list_like(nested):
                    collapsed_nested.append(
                        f"({extract_argument_types(nested)})"
                    )
                else:
                    collapsed_nested.append(
                        _get_argument_readable_type(nested)
                    )
...
def find_matching_fn_abi(
    ...
    fn_identifier: Optional[
        Union[str, Type[FallbackFn], Type[ReceiveFn]]
    ] = None,
    ...
    arg_count_filter = functools.partial(
        filter_by_argument_count, num_arguments
    )
    encoding_filter = functools.partial(
        filter_by_encodability, abi_codec, args, kwargs
    )

    function_candidates = pipe(
        abi, name_filter, arg_count_filter, encoding_filter
    )
...
def get_function_info(
    ...
    if fn_abi is None:
        fn_abi = find_matching_fn_abi(
            contract_abi, abi_codec, fn_name, args, kwargs
        )
...
def parse_block_identifier_int(
...
async def async_parse_block_identifier(
    ...
    if isinstance(block_identifier, int):
        return await async_parse_block_identifier_int(
