...
def get_abi_input_types(abi: ABIFunction) -> List[str]:
    if "inputs" not in abi and (
        abi["type"] == "fallback" or abi["type"] == "receive"
    ):
        return []
    else:
        return [
...
def fallback_func_abi_exists(
    contract_abi: ABI,
) -> List[Union[ABIFunction, ABIEvent]]:
    return filter_by_type("fallback", contract_abi)


def receive_func_abi_exists(
    contract_abi: ABI,
) -> List[Union[ABIFunction, ABIEvent]]:
...
def merge_args_and_kwargs(
    ...
    sorted_arg_names = tuple(
        arg_abi["name"] for arg_abi in function_abi["inputs"]
    )
...
def get_aligned_abi_inputs(
    ...
    return (
        ...
        type(args)(
            _align_abi_input(abi, arg) for abi, arg in zip(input_abis, args)
        ),
    )
...
def length_of_array_type(abi_type: TypeStr) -> int:
    ...
    inner_brackets = (
        re.search(END_BRACKETS_OF_ARRAY_TYPE_REGEX, abi_type)
        .group(0)
        .strip("[]")
    )
...
ARRAY_REGEX = ("^" "[a-zA-Z0-9_]+" "({sub_type})+" "$").format(
    sub_type=SUB_TYPE_REGEX
)
...
def named_tree(
    abi: Iterable[
        Union[ABIFunctionParams, ABIFunction, ABIEvent, Dict[TypeStr, Any]]
    ],
...
def _named_subtree(
    ...
    if abi_type.is_array:
        ...
        items = [
            _named_subtree(*item) for item in zip(abi["components"], data)
        ]
...
async def async_data_tree_map(
    ...
    return await async_recursive_map(
        async_w3, async_map_to_typed_data, data_tree
    )
