...
def _parse_error_with_reverted_prefix(data: str) -> str:
    ...
    try:
        ...
        warnings.warn(
            "Could not decode revert reason as UTF-8", RuntimeWarning
        )
...
def _raise_contract_error(response_error_data: str) -> None:
    ...
    # https://blog.soliditylang.org/2021/04/21/custom-errors/
        ...
        raise ContractCustomError(
            response_error_data, data=response_error_data
        )
...
def raise_transaction_indexing_error_if_indexing(
    ...
    if not isinstance(error, str) and error is not None:
        ...
        if message is not None:
            if all(
                idx_key_phrases in message
                for idx_key_phrases in ("index", "progress")
            ):
