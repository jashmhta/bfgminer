...
class AsyncEthModuleTest:
    ...
    async def test_eth_send_transaction_legacy(
        ...
        assert is_same_address(
            txn["from"], cast(ChecksumAddress, txn_params["from"])
        )
        assert is_same_address(
            txn["to"], cast(ChecksumAddress, txn_params["to"])
        )
        assert txn["value"] == 1
    ...
    async def test_eth_modify_transaction_legacy(
        ...
        assert (
            modified_txn["gasPrice"] == cast(int, txn_params["gasPrice"]) * 2
        )
    ...
    async def test_eth_modify_transaction(
        ...
        assert (
            modified_txn["maxFeePerGas"]
            == cast(Wei, txn_params["maxFeePerGas"]) * 2
        )
    ...
    async def test_async_eth_sign_transaction(
        ...
        signatory_account = async_w3.eth.account.recover_transaction(
            result["raw"]
        )
    ...
    async def test_async_eth_sign_transaction_hex_fees(
        ...
        assert result["tx"]["value"] == txn_params["value"]
        assert result["tx"]["gas"] == txn_params["gas"]
        assert result["tx"]["maxFeePerGas"] == int(
            str(txn_params["maxFeePerGas"]), 16
        )
    ...
    async def test_async_eth_sign_transaction_ens_names(
        ...
        with ens_addresses(
            async_w3, {"unlocked-account.eth": async_unlocked_account}
        ):
            txn_params: TxParams = {
    ...
    async def test_eth_send_transaction(
        ...
        assert (
            txn["maxPriorityFeePerGas"] == txn_params["maxPriorityFeePerGas"]
        )
    ...
    async def test_eth_send_transaction_no_max_fee(
        ...
        assert (
            txn["maxFeePerGas"]
            == maxPriorityFeePerGas + 2 * block["baseFeePerGas"]
        )
    ...
    async def test_eth_max_priority_fee_with_fee_history_calculation_error_dict(
        ...
        async_w3.middleware_onion.remove(
            "fail_max_prio_middleware"
        )  # clean up
    ...
    async def test_eth_get_block_by_number_full_transactions(
        ...
        block = await async_w3.eth.get_block(
            async_block_with_txn["number"], True
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        "unknown_block_num_or_hash", (1234567899999, UNKNOWN_HASH)
    )
    ...
    async def test_eth_get_code_invalid_address(
        ...
        with pytest.raises(InvalidAddress):
            await async_w3.eth.get_code(
                ChecksumAddress(
                    HexAddress(HexStr(async_math_contract.address.lower()))
                )
            )
    ...
    async def test_eth_get_code_with_block_identifier(
        ...
        code = await async_w3.eth.get_code(
            async_emitter_contract.address, block_id
        )
        assert isinstance(code, HexBytes)
        assert len(code) > 0
    ...
    async def test_eth_call_with_override_code(
        ...
        bytes_call_result = await async_w3.eth.call(
            ...
            "latest",
            {
                async_revert_contract.address: {
                    "code": to_bytes(hexstr=override_code)
                }
            },
        )
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            },
            {
                ...
                # with stateDiff
                "stateDiff": {
                    HexStr(f"0x{'00' * 32}"): HexStr(f"0x{'00' * 32}")
                },
            },
        ),
    )
    ...
    async def test_eth_call_offchain_lookup(
        ...
        assert (
            async_w3.codec.decode(["string"], response_caller)[0] == "web3py"
        )
        assert (
            async_w3.codec.decode(["string"], response_function_call)[0]
            == "web3py"
        )
    ...
    async def test_eth_call_offchain_lookup_raises_if_max_redirects_is_less_than_4(
        ...
        async_w3.provider.ccip_read_max_redirects = (
            default_max_redirects  # cleanup
        )
    ...
    async def test_eth_call_continuous_offchain_lookup_raises_with_too_many_requests(
        ...
        with pytest.raises(
            TooManyRequests, match="Too many CCIP read redirects"
        ):
    ...
    async def test_async_eth_get_transaction_receipt_with_log_entry(
        ...
        assert is_dict(receipt)
        assert (
            receipt["blockNumber"] == async_block_with_txn_with_log["number"]
        )
        ...
        assert (
            log_entry["blockNumber"] == async_block_with_txn_with_log["number"]
        )
        assert log_entry["blockHash"] == async_block_with_txn_with_log["hash"]
        assert log_entry["logIndex"] == 0
        assert is_same_address(
            log_entry["address"], async_emitter_contract.address
        )
    ...
    async def test_async_eth_get_logs_without_logs(
        ...
        filter_params: FilterParams = {
            ...
            "toBlock": BlockNumber(
                async_block_with_txn_with_log["number"] - 1
            ),
        }
    ...
    async def test_async_eth_get_storage_at(
        ...
        slot_0 = await async_w3.eth.get_storage_at(
            async_storage_contract_address, 0
        )
        assert slot_0 == HexBytes(f"0x{'00' * 32}")

        slot_1 = await async_w3.eth.get_storage_at(
            async_storage_contract_address, 1
        )
        assert slot_1 == HexBytes(f"0x{'00' * 31}01")

        slot_2 = await async_w3.eth.get_storage_at(
            async_storage_contract_address, 2
        )
        assert slot_2 == HexBytes(f"0x{'00' * 31}02")

        slot_3 = await async_w3.eth.get_storage_at(
            async_storage_contract_address, 3
        )
        ...
        slot_4 = await async_w3.eth.get_storage_at(
            async_storage_contract_address, 4
        )
    ...
    async def test_async_eth_get_storage_at_ens_name(
        ...
        with ens_addresses(
            async_w3, {"storage.eth": async_storage_contract.address}
        ):
    ...
    async def test_async_eth_replace_transaction_legacy(
        ...
        replace_txn_hash = await async_w3.eth.replace_transaction(
            txn_hash, txn_params
        )
    ...
    async def test_async_eth_replace_transaction_underpriced(
        ...
        with pytest.raises(
            ValueError, match="replacement transaction underpriced"
        ):
    ...
    async def test_async_eth_replace_transaction_gas_price_defaulting_strategy_higher(
        ...
        def higher_gas_price_strategy(
            async_w3: "AsyncWeb3", txn: TxParams
        ) -> Wei:
            return two_gwei_in_wei
    ...
    @pytest.mark.asyncio
    async def test_async_eth_new_block_filter(
        self, async_w3: "AsyncWeb3"
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_async_eth_uninstall_filter(
        self, async_w3: "AsyncWeb3"
    ) -> None:
        filter = await async_w3.eth.filter({})
        assert is_string(filter.filter_id)
...
class EthModuleTest:
    ...
    def test_eth_get_transaction_count_invalid_address(
        self, w3: "Web3"
    ) -> None:
        coinbase = w3.eth.coinbase
        with pytest.raises(InvalidAddress):
    ...
    def test_eth_sign_ens_names(
        ...
        with ens_addresses(
            w3, {"unlocked-acct.eth": unlocked_account_dual_type}
        ):
            signature = w3.eth.sign(
    ...
    def test_eth_sign_transaction_ens_names(
        ...
        with ens_addresses(w3, {"unlocked-account.eth": unlocked_account}):
            ...
            result = w3.eth.sign_transaction(txn_params)
            signatory_account = w3.eth.account.recover_transaction(
                result["raw"]
            )
            assert unlocked_account == signatory_account
    ...
    def test_eth_send_transaction_with_nonce(
        ...
        txn_params: TxParams = {
            ...
            "nonce": Nonce(
                w3.eth.get_transaction_count(unlocked_account, "pending")
            ),
        }
        txn_hash = w3.eth.send_transaction(txn_params)
        txn = w3.eth.get_transaction(txn_hash)
    ...
    def test_eth_call_offchain_lookup_raises_when_ccip_read_is_disabled(
        ...
        with pytest.raises(OffchainLookup) as excinfo:
            offchain_lookup_contract.caller(
                ccip_read_enabled=False
            ).testOffchainLookup(OFFCHAIN_LOOKUP_TEST_DATA)
        assert excinfo.value.data == return_data
    ...
    def test_eth_getTransactionByBlockHashAndIndex(
        ...
        transaction = w3.eth.get_transaction_by_block(
            block_with_txn["hash"], 0
        )
        assert is_dict(transaction)
        assert transaction["hash"] == HexBytes(mined_txn_hash)
    ...
    def test_eth_get_raw_transaction_by_block(
        ...
        last_pending_txn_index = (
            len(w3.eth.get_block("pending")["transactions"]) - 1
        )
