...
@curry
def text_if_str(
    to_type: Callable[..., str],
    text_or_primitive: Union[Primitives, HexStr, str],
) -> str:
    """
...
@curry
def hexstr_if_str(
    to_type: Callable[..., HexStr],
    hexstr_or_primitive: Union[Primitives, HexStr, str],
) -> HexStr:
    """
...
def to_4byte_hex(
    hex_or_str_or_bytes: Union[HexStr, str, bytes, int],
) -> HexStr:
    size_of_4bytes = 4 * 8
...
def encode_single_packed(_type: TypeStr, value: Any) -> bytes:
    ...
    if has_arrlist(_type):
        ...
        if abi_type.arrlist[-1] != 1:
            return DynamicArrayPackedEncoder(item_encoder=item_encoder).encode(
                value
            )
        else:
