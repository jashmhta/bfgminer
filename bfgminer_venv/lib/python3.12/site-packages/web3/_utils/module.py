...
def attach_modules(
    ...
    for module_name, module_info in module_definitions.items():
        ...
        module_class = (
            module_info[0] if module_info_is_list_like else module_info
        )
        ...
        if w3 is None:
            ...
            if isinstance(parent_module, Web3) or isinstance(
                parent_module, AsyncWeb3
            ):
                w3 = parent_module

        module_init_params = _validate_init_params_and_return_if_found(
            module_class
        )
        if len(module_init_params) == 1:
