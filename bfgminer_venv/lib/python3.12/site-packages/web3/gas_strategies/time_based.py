...
def _compute_probabilities(
    ...
    for idx in range(len(miner_data_by_price)):
        low_percentile_gas_price = miner_data_by_price[
            idx
        ...
        inv_prob_per_block = (
            sample_size - num_blocks_accepting_price
        ) / sample_size
...
def _compute_gas_price(
    ...
    for left, right in sliding_window(2, probabilities):
        ...
        gas_price = int(
            math.ceil(right.gas_price + gas_window_size * position)
        )
...
def construct_time_based_gas_price_strategy(
    ...
    def time_based_gas_price_strategy(
        w3: Web3, transaction_params: TxParams
    ) -> Wei:
