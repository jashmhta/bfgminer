...
class AsyncEth(BaseEth):
    ...
    async def fee_history(
        ...
        return await self._fee_history(
    ...
    async def call(
        ...
        ccip_read_enabled_on_provider = (
            self.w3.provider.global_ccip_read_enabled
        )
        if (
            # default conditions:
            ccip_read_enabled_on_provider
    ...
    _get_raw_transaction: Method[Callable[[_Hash32], Awaitable[HexBytes]]] = (
        Method(RPC.eth_getRawTransactionByHash, mungers=[default_root_munger])
    )
    ...
    async def send_raw_transaction(
        self, transaction: Union[HexStr, bytes]
    ) -> HexBytes:
    ...
    _get_logs: Method[
        Callable[[FilterParams], Awaitable[List[LogReceipt]]]
    ...
    _sign_typed_data: Method[
        Callable[
            [Union[Address, ChecksumAddress, ENS], str], Awaitable[HexStr]
        ]
    ] = Method(
    ...
    filter: Method[
        Callable[
            [Optional[Union[str, FilterParams, HexStr]]],
            Awaitable[AsyncFilter],
        ]
    ] = Method(
    ...
    async def subscribe(
        ...
        return await self._subscribe_with_args(
