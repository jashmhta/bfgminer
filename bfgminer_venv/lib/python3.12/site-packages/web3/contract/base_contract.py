...
class BaseContractEvent:
    ...
    def _get_event_abi(cls) -> ABIEvent:
        return find_matching_event_abi(
            cls.contract_abi, event_name=cls.event_name
        )
    ...
    def check_for_forbidden_api_filter_arguments(
        ...
        name_indexed_inputs = {
            _input["name"]: _input for _input in event_abi["inputs"]
        }
        ...
        for filter_name, filter_value in _filters.items():
            ...
            if is_list_like(filter_value) and is_dynamic_sized_type(
                _input["type"]
            ):
                raise TypeError(
    ...
    def _set_up_filter_builder(
        ...
        filter_builder: Union[
            EventFilterBuilder, AsyncEventFilterBuilder
        ] = None,
    ) -> None:
...
class BaseContractFunction:
    ...
    def _set_function_info(self) -> None:
        ...
        self.arguments = merge_args_and_kwargs(
    ...
    def _estimate_gas(
        ...
        if "data" in estimate_gas_transaction:
            raise ValueError(
                "Cannot set 'data' field in estimate_gas transaction"
            )
    ...
    def _encode_transaction_data(cls) -> HexStr:
        return add_0x_prefix(
            encode_abi(cls.w3, cls.abi, cls.arguments, cls.selector)
        )
    ...
    def _encode_constructor_data(
        ...
        if constructor_abi:
            ...
            deploy_data = add_0x_prefix(
                encode_abi(
                    cls.w3, constructor_abi, arguments, data=cls.bytecode
                )
            )
        else:
...
class BaseContractConstructor:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.data_in_transaction = self._encode_data_in_transaction(
            *args, **kwargs
        )
    ...
    def _encode_data_in_transaction(self, *args: Any, **kwargs: Any) -> HexStr:
        ...
        if constructor_abi:
            ...
            data = add_0x_prefix(
                encode_abi(
                    self.w3, constructor_abi, arguments, data=self.bytecode
                )
            )
        else:
