...
def block_ranges(
    ...
    return (
        ...
        for from_block, to_block in segment_count(
...
def get_logs_multipart(
    ...
    address: Union[
        Address, ChecksumAddress, List[Union[Address, ChecksumAddress]]
    ],
...
def block_hashes_in_range(
    ...
    for block_number in range(from_block, to_block + 1):
        yield getattr(
            w3.eth.get_block(BlockNumber(block_number)), "hash", None
        )
    ...
    async def to_block(self) -> BlockNumber:
        if self._to_block is None or self._to_block == "latest":
            ...
            to_block = BlockNumber(
                hex_to_integer(cast(HexStr, self._to_block))
            )
        else:
            to_block = self._to_block
