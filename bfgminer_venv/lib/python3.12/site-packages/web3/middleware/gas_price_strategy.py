...
def validate_transaction_params(
    ...
    if (
        ...
        transaction = assoc(
            transaction,
            "gasPrice",
            to_hex_if_integer(strategy_based_gas_price),
        )

    # legacy and dynamic fee tx variables used:
    if "gasPrice" in transaction and any_in_dict(
        DYNAMIC_FEE_TXN_PARAMS, transaction
    ):
    ...
    # dynamic fee txn - no max fee:
        ...
        and "maxPriorityFeePerGas" in transaction
    ):
