...
class RequestManager:
    ...
    def __init__(
        ...
        middlewares: Optional[
            Union[
                Sequence[Tuple[Middleware, str]],
                Sequence[Tuple[AsyncMiddleware, str]],
            ]
        ] = None,
    ) -> None:
        self.w3 = w3
    ...
    @provider.setter
    def provider(
        self, provider: Union["BaseProvider", "AsyncBaseProvider"]
    ) -> None:
        ...
        # Format and validate results
            ...
            if response.get("result", False) in NULL_RESPONSES:
                apply_null_result_formatters(
                    null_result_formatters, response, params
                )
    ...
    async def _process_ws_response(self, response: RPCResponse) -> RPCResponse:
        provider = cast(PersistentConnectionProvider, self._provider)
        request_info = (
            self._request_processor.get_request_information_for_response(
                response
            )
        )
        ...
        if request_info is None:
            self.logger.debug(
                "No cache key found for response, returning raw response"
            )
            return response
        else:
            if (
                ...
                if (
                    cache_key
                    not in self._request_processor._request_information_cache
                ):
...
class _AsyncPersistentMessageStream:
    ...
    def __init__(
        self, manager: RequestManager, *args: Any, **kwargs: Any
    ) -> None:
