...
def lookup_property(
    property, value, positive, source=None, posix=False, encoding=0
):
    "Looks up a property."
    ...
    for property in ("GC", "SCRIPT", "BLOCK"):
        ...
        if val_id is not None:
            return Property(
                (prop_id << 16) | val_id, positive, encoding=encoding
            )
    ...
    if value.startswith("IS"):
        ...
        if prop:
            ...
            if "YES" in value_dict:
                return Property(
                    (prop_id << 16) | 1, positive, encoding=encoding
                )
...
class Property(RegexBase):
    ...
    def rebuild(self, positive, case_flags, zerowidth):
        return Property(
            self.value, positive, case_flags, zerowidth, self.encoding
        )
    ...
    def optimise(self, info, reverse, in_set=False):
        ...
        for m in items:
            if isinstance(m, Property):
                properties[m.positive].add(
                    (m.value, m.case_flags, m.zerowidth)
                )
