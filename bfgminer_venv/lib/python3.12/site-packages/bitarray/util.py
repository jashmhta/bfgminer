...
def rindex(__a, __sub_bitarray=1, __start=0, __stop=sys.maxsize):
    ...
    warn(
        "rindex() is deprecated and will be removed in bitarray 3.0 - "
...
def make_endian(__a, endian):
    ...
    warn(
        "make_endian() is deprecated and will be removed in bitarray 3.0 - "
...
def strip(__a, mode="right"):
    ...
    if mode not in ("left", "right", "both"):
        raise ValueError(
            "mode must be 'left', 'right' or 'both', got %r" % mode
        )
...
def int2ba(__i, length=None, endian=None, signed=False):
    ...
    if signed:
        ...
        if length and __i >= (1 << length):
            raise OverflowError(
                "unsigned integer not in range(0, %d), "
                "got %d" % (1 << length, __i)
            )
