        ...
        for a in self.randombitarrays(start=1):
            ...
            for _ in range(10):
                ...
                lb = (
                    randrange(10)
                    if step is None
                    else self.calc_slicelength(s, la)
                )
                b = bitarray(lb)
                c = bitarray(a)
                c[s] = b
...
class IndexTests(unittest.TestCase, Util):
        ...
        for x in bytearray([0]), b"\0", "A":
            ...
            for n in range(5):
                ...
                for start in range(-5, 5):
                    self.assertEqual(
                        z.find(empty, start), self.find_empty(n, start)
                    )
                    self.assertEqual(
                        z.rfind(empty, start),
                    ...
                    for stop in range(-5, 5):
                        self.assertEqual(
                            z.find(empty, start, stop),
                            self.find_empty(n, start, stop),
                        )
                        self.assertEqual(
                            z.rfind(empty, start, stop),
    ...
    def test_empty(self):
        ...
        for n in range(5):
            ...
            for r in 0, 1:
                self.assertEqual(
                    z.find(empty, right=r), self.find_empty(n, right=r)
                )

                for start in range(-5, 5):
                    self.assertEqual(
                        z.find(empty, start, right=r),
                        self.find_empty(n, start, right=r),
                    )

                    for stop in range(-5, 5):
                        self.assertEqual(
                            z.find(empty, start, stop, r),
...
class SearchTests(unittest.TestCase, Util):
    ...
    def test_simple(self):
        ...
        for limit in range(10):
            self.assertEqual(
                a.search(bitarray("011"), limit), [6, 11, 20][:limit]
            )
    ...
    def test_itersearch_empty(self):
        ...
        for start, stop, right, res in [
            ...
            self.assertEqual(
                list(a.itersearch(empty, start, stop, right)), res
            )
    ...
    def test_itersearch_random(self):
        for _ in range(500):
            ...
            if b:
                plst = [
                    i + k for k in range(len(aa)) if aa[k : k + len(b)] == b
                ]
...
class BytesTests(unittest.TestCase, Util):
    ...
    def test_frombytes_simple(self):
        ...
        b.frombytes(b"BC")
        self.assertEQUAL(
            b, bitarray("01000001 01000010 01000011", endian="big")
        )
...
class DecodeTreeTests(unittest.TestCase, Util):
    ...
    def test_complete(self):
        ...
        dt = decodetree(
            {"a": bitarray("0"), "b": bitarray("11"), "c": bitarray("10")}
        )
        self.assertTrue(dt.complete())
    ...
    def test_large(self):
        d = {
            i: bitarray(bool((1 << j) & i) for j in range(10))
            for i in range(1024)
        }
        t = decodetree(d)
...
class BufferImportTests(unittest.TestCase, Util):
    ...
    def test_bytearray(self):
        ...
        for n in 7, 9:
            self.assertRaises(
                BufferError, a.__setitem__, slice(8, 16), bitarray(n)
            )
        b[1] = b[2] = 255
        self.assertEqual(b, bytearray(100 * [255]))
    ...
    def test_bitarray(self):
        ...
        self.assertRaisesMessage(
            BufferError, "cannot resize imported buffer", b.pop
        )
        self.check_obj(a)
        self.check_obj(b)
    ...
    def test_bitarray_shared_sections(self):
        ...
        self.assertEqual(
            buffer_info(c, "address"), buffer_info(a, "address") + 0x200
        )
