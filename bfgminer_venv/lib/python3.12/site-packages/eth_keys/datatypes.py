...
class BaseKey(ByteString, collections.abc.Hashable):
    ...
    def to_hex(self) -> str:
        return "0x" + codecs.decode(
            codecs.encode(self._raw_key, "hex"), "ascii"
        )
...
class BaseSignature(ByteString, LazyBackend, ABC):
    ...
    def verify_msg_hash(
        self, message_hash: bytes, public_key: PublicKey
    ) -> bool:
...
class Signature(BaseSignature):
    ...
    def recover_public_key_from_msg_hash(
        self, message_hash: bytes
    ) -> PublicKey:
...
class NonRecoverableSignature(BaseSignature):
    ...
    def __init__(
        ...
        if signature_bytes is None and rs is None:
            raise TypeError(
                "You must provide one of `signature_bytes` or `vr`"
            )
        elif signature_bytes:
