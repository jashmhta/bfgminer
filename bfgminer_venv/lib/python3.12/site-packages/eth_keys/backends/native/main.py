...
class NativeECCBackend(BaseECCBackend):
    ...
    def ecdsa_sign_non_recoverable(
        ...
        _, signature_r, signature_s = ecdsa_raw_sign(
            msg_hash, private_key.to_bytes()
        )
        signature = NonRecoverableSignature(
            rs=(signature_r, signature_s), backend=self
        )
        return signature
    ...
    def ecdsa_recover(
        self, msg_hash: bytes, signature: Signature
    ) -> PublicKey:
    ...
    def decompress_public_key_bytes(
        self, compressed_public_key_bytes: bytes
    ) -> bytes:
        return decompress_public_key(compressed_public_key_bytes)

    def compress_public_key_bytes(
        self, uncompressed_public_key_bytes: bytes
    ) -> bytes:
